...
export class AddFriendService {
  ...
  async addFriendByReceiverId(userid: number, recvFriendRequestDto: FriendRequestDto): Promise<void> {
    ...
    if (await this.userRepository.findOne({ where: { id: recvFriendRequestDto.recvFriendRequestUserId } }) === undefined) {
      throw new BadRequestException (`존재하지 않는 사용자입니다.`);
    }
    if (await this.friendRepository.findOne({ where: { sendFriendRequestUserId: userid, recvFriendRequestUserId: recvFriendRequestDto.recvFriendRequestUserId } }) !== undefined) {
